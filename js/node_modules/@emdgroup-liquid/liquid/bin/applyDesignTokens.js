...
function getColorTokenValue(variant, styles) {
    if (variant.styles?.fill) {
        ...
        const referenceName = baseColorName.replaceAll(/[a-z]/g, '').toLowerCase() +
                ...
                if (variants) {
                    variants.forEach((variant) => {
                        ...
                        if (variant.children) {
                            ...
                            const colorName = `${groupName}${variantName === 'default' ? '' : `-${variantName}`}`;
...
function parseShadows(items) {
    ...
    for (const item of items) {
        shadows[item.name.split(' ')[0].toLowerCase()] = item.effects
            ...
            return `${pxToRem(effect.offset.x)} ${pxToRem(effect.offset.y)} ${pxToRem(effect.radius)} rgba(${Math.round(col.r * 255)}, ${Math.round(col.g * 255)}, ${Math.round(col.b * 255)}, ${Math.round(col.a * 100) / 100})`;
        })
            .join(', ');
    }
    return shadows;
...
function parseColors(items, styles) {
    ...
    for (const item of items) {
        ...
        if (item.children) {
            ...
            const colorName = colorShortName +
...
function parseTypography(items, styles) {
    ...
    items.forEach((item) => {
        ...
        const typoName = styleName
            ...
            .replace(/^pg-/, 'body-')
            .replace(/^caption-/, 'cap-');
        const { fontFamily, fontSize, fontWeight, lineHeightPercentFontSize } = item.style;
        const baseFontName = fontFamily === 'Merck'
            ? 'MWeb'
            : fontFamily.includes(' ')
                ? `'${fontFamily}'`
                : fontFamily;
...
function parseSpacings(items) {
    const spacings = {};
    for (const item of items) {
        spacings[item.name.split('$spacing-')[1]] = pxToRem(item.absoluteBoundingBox.height);
    }
    return spacings;
}
async function getTokensFromFigma(figmaFileURL) {
    let figmaId;
    let nodeId;
    try {
    ...
    const result = await new Promise((resolve, reject) => {
        https
            .get(`https://api.figma.com/v1/files/${figmaId}/nodes?ids=${nodeId}`, {
            headers: {
    ...
    const { document, styles } = result.nodes[nodeId];
    const { children: figmaData } = document;
    const tokens = {
        themes: parseThemes(figmaData.find(({ name }) => name === 'Themes').children, styles),
        shadows: parseShadows(figmaData.find((child) => child.name === 'Shadows').children),
        spacings: parseSpacings(figmaData.find((child) => child.name === 'Spacings').children),
        colors: {
            ...parseColors(figmaData.find((child) => child.name === 'Colors').children, styles),
        },
        ...
        borderRadii: parseBorderRadii(figmaData.find((child) => child.name === 'Border Radius').children),
    };
    return tokens;
}
function boxShadowToDropShadow(boxShadow) {
...
function generateShadows(tokens) {
    return ensureWriteFile(join(stylesDir, 'shadows/shadows.css'), '/* autogenerated */\n:root {\n' +
        Object.keys(tokens)
            .sort()
        ...
        Object.keys(tokens)
            .sort()
            .map((key) => `  --ld-drop-shadow-${key}: ${boxShadowToDropShadow(tokens[key])};`)
            .join('\n') +
        '\n}\n', 'utf8');
}
function generateColors(colorTokens) {
    ...
    return ensureWriteFile(join(stylesDir, 'colors/colors.css'), ['/* autogenerated */', ':root {', ...colorVariables.sort(), '}', ''].join('\n'), 'utf8');
}
function generateSpacings(tokens) {
    return ensureWriteFile(join(stylesDir, 'spacings/spacings.css'), '/* autogenerated */\n:root {\n' +
        Object.keys(tokens)
...
function generateTheming(themes) {
    ...
    Object.keys(themes).forEach((themeName) => {
        ...
        Object.keys(theme).forEach((colorGroupName) => {
            ...
            const variableValue = colorGroup.indexOf('rgb') === 0
                ? colorGroup
                : `var(--ld-col-${colorGroup})`;
            themeColorVariables.push(`  --ld-thm-${themeName}-${colorGroupName}: ${variableValue};`);
            currentThemeColorVariables.push(`  --ld-thm-${colorGroupName}: var(--ld-thm-${themeName}-${colorGroupName});`);
            if (theme.default) {
                defaultThemeColorVariables.push(`  --ld-thm-${colorGroupName}: var(--ld-thm-${themeName}-${colorGroupName});`);
            }
        });
    ...
    return ensureWriteFile(join(stylesDir, 'theming/theming.css'), [
        '/* autogenerated */',
        ':root {',
...
function generateTypography(tokens) {
    ...
    const [, { fontFamily: bodyFontFamily }] = tokenEntries.find(([key]) => key.startsWith('body'));
    const nonBodyTypoEntry = tokenEntries.find(([, value]) => value.fontFamily !== bodyFontFamily);
    return ensureWriteFile(join(stylesDir, 'typography/typography.css'), '/* autogenerated */\n:root {\n' +
        ...
        Object.keys(tokens)
            ...
            return `  --ld-typo-${key}: ${val.fontWeight ? `${val.fontWeight} ` : ''}${val.fontSize}/${val.lineHeight} ${val.fontFamily};`;
        })
            .join('\n') +
        '\n}\n', 'utf8');
}
function generateBorderRadii(tokens) {
    return ensureWriteFile(join(stylesDir, 'border-radius/border-radius.css'), '/* autogenerated */\n:root {\n' +
        Object.keys(tokens)
            .sort((key) => parseInt(key))
...
function generateJSONTokenFile(tokenCollection) {
    return ensureWriteFile(join(stylesDir, 'design-tokens.json'), JSON.stringify(tokenCollection, null, 2), 'utf8');
}
async function applyDesignTokens(figmaFileURL = process.env.FIGMA_FILE_URL ||
