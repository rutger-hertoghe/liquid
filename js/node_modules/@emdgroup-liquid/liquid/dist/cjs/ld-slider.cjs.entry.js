...
linear-gradient(
  ...
  transparent 0
            ...
            .join(',') }) }, this.values.map((value, index$1) => {
      var _a;
      return (index.h(index.Fragment, null, index.h("ld-sr-only", { id: `${this.idPrefix}-label-${index$1}`, key: `label-${index$1}`, part: "label" }, this.values.length === 2
        ? index$1 === 0
          ? this.labelFrom
          : this.labelTo
        : `${this.labelValue} ${index$1 + 1}`), index.h("input", { "aria-disabled": this.disabled || this.ariaDisabled === 'true'
          ? 'true'
          : undefined, "aria-labelledby": `${this.idPrefix}-label-${index$1}`, "aria-valuetext": this.unit ? value + this.unit : undefined, class: "ld-slider__input", disabled: this.disabled, id: `${this.idPrefix}-value-${index$1}`, key: `input-${index$1}`, max: this.max, min: this.min, onInput: (ev) => this.handleInput(ev, index$1), onKeyDown: (ev) => this.handleKeyDown(ev, index$1), part: "input focusable", ref: index$1 === 0
          ? (ref) => {
        ...
        value === this.max && this.values[index$1 - 1] === this.max
          ? {
            zIndex: '2',
          }
          : undefined, tabindex: this.ldTabindex, type: "range", value: value }), index.h("output", { class: getClassNames.getClassNames([
          'ld-slider__output',
          !this.hideValues && 'ld-slider__output--permanent',
        ]), htmlFor: `${this.idPrefix}-value-${index$1}`, key: `output-${index$1}`, part: "output", style: {
          '--now': `var(--value${index$1})`,
          '--unit': `"${(_a = this.unit) !== null && _a !== void 0 ? _a : ''}"`,
        } })));
    }), this.indicators &&
      this.steps.map((step) => (index.h("div", { class: "ld-slider__indicator", key: `indicator-${step}`, part: "indicator", style: { '--now': String(step) } }))), !this.hideValueLabels &&
      this.valueLabels.map((valueLabel, index$1) => (index$1 === 0 ||
        index$1 === this.valueLabels.length - 1 ||
        !this.hideStopLabels) && (index.h("div", { class: getClassNames.getClassNames([
          'ld-slider__value-label',
          index$1 === 0 && 'ld-slider__value-label--first',
          index$1 === this.valueLabels.length - 1 &&
...
LdSlider.style = ldSliderShadowCss;

exports.ld_slider = LdSlider;
