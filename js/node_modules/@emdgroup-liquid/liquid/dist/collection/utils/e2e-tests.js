...
export const getPageWithContent = async (content, config) => {
  const page = (await newE2EPage({
    html: (config === null || config === void 0 ? void 0 : config.notWrapped)
      ? content
  ...
  await page.addStyleTag({
    content: `${(config === null || config === void 0 ? void 0 : config.disableAllTransitions) ? disableAllTransitionsStyles : ''}
    body {
      margin: 0;
      ${(config === null || config === void 0 ? void 0 : config.bgColor) ? `background-color: ${config.bgColor};` : ''}
      height: 600px;
    }
    *:focus,
    ::part(focusable) {
  ...
  if (config === null || config === void 0 ? void 0 : config.components) {
    await Promise.all([config.components].flat().map((component) => {
      const cssFileName = component.COMPILER_META.styles[0].externalStyles[0].relativePath;
      return page.addStyleTag({ path: `./dist/css/${cssFileName}` });
    }));
  }
  if (config === null || config === void 0 ? void 0 : config.reducedMotion) {
...
export const analyzeAccessibility = async (page, config = {}) => {
  const options = Object.assign({ rules: {} }, config.options);
  ...
  return page.evaluate(async (axeOptions, spec) => {
...
function getInvalidRuleInfo(rule) {
  return `[rule id: ${rule.id}] ${printReceived(rule.help)} on ${rule.nodes.length} nodes\r\n${rule.nodes.map(getInvalidNodeInfo).join('\n')}`;
}
...
expect.extend({
  toHaveNoAccessibilityIssues(accessibilityReport, options) {
    ...
    const filteredViolations = accessibilityReport.violations.filter((violation) => violation.nodes.length > 0);
    const filteredIncomplete = accessibilityReport.incomplete.filter((incomplete) => incomplete.nodes.length > 0);
    ...
    if (finalOptions.incompleteThreshold !== false &&
    ...
    const pass = filteredViolations.length <= finalOptions.violationsThreshold &&
