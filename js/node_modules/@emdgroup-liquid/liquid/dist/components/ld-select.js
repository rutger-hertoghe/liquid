...
const LdSelect$1 = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    ...
    this.updateTriggerMoreIndicator = (refresh = false) => {
      if (!this.multiple || !this.maxRows)
        return;
      if (refresh)
        this.hasMore = false;
    ...
    this.isLdOption = (el) => { var _a; return ['LD-OPTION', 'LD-OPTION-INTERNAL'].includes((_a = el) === null || _a === void 0 ? void 0 : _a.tagName); };
    ...
    this.updatePopperTheme = () => {
      ...
      setTimeout(() => {
        ...
        this.theme = (_a = themeEl.classList
          .toString()
          .split(' ')
          .find((cl) => cl.startsWith('ld-theme-'))) === null || _a === void 0 ? void 0 : _a.substring(9);
      });
    };
    this.updatePopper = () => {
      if (!this.popper)
    ...
    this.initPopper = () => {
      const customTetherOptions = typeof this.tetherOptions === 'string'
        ...
        : this.tetherOptions;
      const tetherOptions = Object.assign({ classPrefix: 'ld-tether', element: this.listboxRef, target: this.selectRef, attachment: 'top left', targetAttachment: 'bottom left', offset: this.mode ? '-4px 0' : '0 0', constraints: [
          {
            to: 'window',
            pin: true,
          },
    ...
    this.initOptions = () => {
      ...
      if (!initialized) {
        ...
        children.forEach((ldOption) => {
          ...
          internalOptionsHTML += `<ld-option-internal${classStr ? ' class="' + classStr + '"' : ''}${this.multiple ? ' mode="checkbox"' : ''}${this.size ? ' size="' + this.size + '"' : ''}${this.preventDeselection ? ' prevent-deselection' : ''}${ldOption.selected ? ' selected' : ''}${ldOption.hidden ? ' hidden' : ''}${ldOption.value ? ' value="' + ldOption.value + '"' : ''}${ldOption.disabled ? ' disabled' : ''}>${ldOption.innerHTML.replaceAll(/<ld-icon (.|\n|\r)*slot="icon"(.|\n|\r)*>(.|\n|\r)*<\/ld-icon>/g, '')}</ld-option-internal>`;
        });
    ...
    this.handleSlotChange = (mutationsList) => {
    ...
    this.getFilterInput = () => this.listboxRef.shadowRoot.querySelector('.ld-select-popper__filter-input');
    this.togglePopper = () => {
    ...
    this.expandAndFocus = () => {
      ...
      setTimeout(() => {
        ...
        if (!this.multiple) {
          ...
          toFocus = (_a = Array.from(this.listboxRef.querySelectorAll('ld-option-internal'))
    ...
    this.selectAndFocus = (ev, ldOption) => {
      if (!ldOption)
        return;
    ...
    this.resetFilter = () => {
      ...
      if (!filterInput)
        return;
      filterInput.value = '';
      const options = this.internalOptionsContainerRef.querySelectorAll('ld-option-internal');
      options.forEach((ldOption) => {
    ...
    this.handleFocusEvent = (ev) => {
      ...
      if (ev.relatedTarget === null ||
        ...
        closest('ld-select', ev.relatedTarget) === this.el) {
    ...
    this.handleTriggerClick = (ev) => {
      ...
      setTimeout(() => {
        var _a;
        (_a = this.getFilterInput()) === null || _a === void 0 ? void 0 : _a.focus();
      });
    };
    ...
    this.handleClearSingleClick = (ev, optionValue) => {
      ev.preventDefault();
      ev.stopImmediatePropagation();
      if (this.disabled || this.ariaDisabled)
        return;
  ...
  isOverflowing() {
    /* istanbul ignore next */
    return (this.selectionListRef.scrollHeight >
  ...
  handleKeyDown(ev) {
    ...
    const filterHasFocus = this.filter &&
      ((_a = this.listboxRef) === null || _a === void 0 ? void 0 : _a.shadowRoot.activeElement) === this.getFilterInput();
      ...
      case 'ArrowDown': {
        ...
        if (document.activeElement === this.el || filterHasFocus) {
          if (this.filter && !filterHasFocus) {
            ...
            const nextLdOption = Array.from(this.listboxRef.querySelectorAll('ld-option-internal')).find((ldOption) => !ldOption.hidden);
  ...
  disconnectedCallback() {
    ...
    (_c = this.listboxRef) === null || _c === void 0 ? void 0 : _c.remove();
  }
  render() {
    var _a, _b;
    ...
    return (h(Host, null, h("div", { class: getClassNames(cl), "aria-disabled": this.disabled || this.ariaDisabled, part: "root", onBlur: this.handleFocusEvent, onFocusout: this.handleFocusEvent, style: this.expanded
        ? {
          zIndex: '2147483647', // Highest possible z-index
        }
        : undefined }, this.renderHiddenInput && h("slot", { name: "hidden" }), h("div", { class: "ld-select__slot-container", part: "slot-container" }, h("slot", null)), h("div", { class: "ld-select__select", part: "select", ref: (el) => (this.selectRef = el) }, h("div", { class: getClassNames(triggerCl), role: "button", part: "btn-trigger focusable", tabindex: this.disabled && !this.ariaDisabled
        ? undefined
        : this.ldTabindex, "aria-disabled": this.disabled || this.ariaDisabled ? 'true' : 'false', "aria-haspopup": "listbox", "aria-expanded": this.expanded ? 'true' : 'false', "aria-label": triggerText, onClick: this.handleTriggerClick, ref: (el) => (this.triggerRef = el) }, this.multiple && this.selected.length ? (h("div", { class: "ld-select__selection-list-container", part: "selection-list-container" }, h("ul", { class: "ld-select__selection-list", part: "selection-list", "aria-label": "Selected options", ref: (el) => (this.selectionListRef = el), style: {
        maxHeight: this.maxRows && this.maxRows > 0
          ? `${this.maxRows * 1.75}rem`
          : undefined,
      } }, this.selected.map((selection, index) => {
      return (h("li", { key: index, class: "ld-select__selection-list-item", style: { order: index + 1 + '' }, part: "selection-list-item" }, h("label", { class: "ld-select__selection-label" }, h("span", { class: "ld-select__selection-label-text", title: selection.text, part: "selection-label-text" }, selection.text), h("button", { disabled: this.disabled || this.ariaDisabled
          ? true
        ...
        } }, h("svg", { class: "ld-select__btn-clear-single-icon", part: "icon-clear-single", fill: "none", viewBox: "0 0 12 12" }, h("title", null, "Clear"), h("path", { stroke: "#fff", "stroke-linecap": "round", "stroke-linejoin": "round", "stroke-width": "2", d: "M2 2l8 8M2 10l8-8" }))), h("span", { class: "ld-select__selection-label-bg", part: "selection-label-bg" }))));
    })))) : (h("span", { class: "ld-select__btn-trigger-text-wrapper", title: triggerText, part: "trigger-text-wrapper" }, h("span", { class: "ld-select__btn-trigger-text", part: "trigger-text" }, triggerText))), ((_b = this.selected) === null || _b === void 0 ? void 0 : _b.length) && this.multiple ? (h("button", { class: "ld-select__btn-clear", disabled: this.disabled || this.ariaDisabled ? true : undefined, onClick: this.handleClearClick, ref: (el) => (this.btnClearRef = el), part: "btn-clear focusable" }, h("svg", { class: "ld-select__btn-clear-icon", fill: "none", viewBox: "0 0 21 20", part: "icon-clear" }, h("title", null, "Clear all"), h("path", { fill: "currentColor", "fill-rule": "evenodd", d: "M10 20a10 10 0 100-20 10 10 0 000 20z", "clip-rule": "evenodd" }), h("path", { stroke: "#fff", "stroke-linecap": "round", "stroke-linejoin": "round", "stroke-width": "2", d: "M6.67 6.67l6.67 6.66M6.67 13.33l6.67-6.66" })))) : (''), h("slot", { name: "icon" }), !this.hasCustomIcon && (
    /* custom icon arrow-down */
    h("svg", { class: getClassNames(triggerIconCl), role: 'presentation', viewBox: "0 0 16 16", part: "trigger-icon" }, h("path", { stroke: "currentColor", "stroke-linecap": "round", "stroke-linejoin": "round", "stroke-width": "3", d: "M3 6l5 4 5-4" }))))), h("ld-select-popper", { detached: detached, expanded: this.expanded, filter: this.filter, filterPlaceholder: this.filterPlaceholder, onBlur: this.handleFocusEvent, onFocusout: this.handleFocusEvent, onLdselectfilterchange: this.handleFilterChange, popperClass: this.popperClass, ref: (el) => (this.listboxRef = el), role: "listbox", size: this.size, theme: this.theme }, h("div", { ref: (el) => (this.internalOptionsContainerRef = el), innerHTML: this.internalOptionsHTML, part: "options-container" })))));
  }
  get el() { return this; }
  ...
  static get style() { return ldSelectCss; }
}, [1, "ld-select", {
    "autofocus": [516],
    "disabled": [4],
    "form": [1],
    ...
    "hasMore": [32],
    "initialized": [32],
    "internalOptionsHTML": [32],
    "renderHiddenInput": [32],
    "theme": [32],
    "typeAheadQuery": [32],
    "typeAheadTimeout": [32],
    "focusInner": [64]
  }, [[9, "resize", "handleWindowResize"], [9, "ldoptionselect", "handleSelect"], [8, "keydown", "handleKeyDown"], [8, "click", "handleClickOutside"], [9, "touchend", "handleTouchOutside"]]]);
function defineCustomElement$1() {
  if (typeof customElements === "undefined") {
    return;
  }
  const components = ["ld-select", "ld-select-popper"];
  components.forEach(tagName => { switch (tagName) {
    case "ld-select":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, LdSelect$1);
      }
      break;
...
const LdSelect = LdSelect$1;
const defineCustomElement = defineCustomElement$1;

export { LdSelect, defineCustomElement };
